...
int main(void)
{
    ...
    Model model = LoadModel("resources/nissan sunny USB.glb");                 // Load OBJ model
    Texture2D texture = LoadTexture("resources/models/obj/turret_diffuse.png"); // Load model texture                 // Load model
    model.materials[0].maps[MATERIAL_MAP_DIFFUSE].texture = texture;            // Set map diffuse texture
    if (model.meshes == NULL)
    {
        printf("No se cargo el modelo, fallo algo");
        return 0;
    }
    ...
    while (!WindowShouldClose())    // Detect window close button or ESC key
    {
        ...
        if (IsKeyDown(KEY_Q))
        {
            camera.position.y += 1.0f * camSpeed * GetFrameTime();
        }
        if (IsKeyDown(KEY_E))
        {
            camera.position.y += -1.0f * camSpeed * GetFrameTime();
        ...
        float yaw = -mousedelta.y * 0.1 * GetFrameTime();
